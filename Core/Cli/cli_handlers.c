...
BaseType_t prvSetCommand(char *pcWriteBuffer, size_t xWriteBufferLen,
  ...
  // Extract parameters
  pcParameter1 =
      FreeRTOS_CLIGetParameter(pcCommandString, 1, &lParameter1StringLength);
  ...
  pcParameter2 =
      FreeRTOS_CLIGetParameter(pcCommandString, 2, &lParameter2StringLength);
...
BaseType_t prvGetCommand(char *pcWriteBuffer, size_t xWriteBufferLen,
                         const char *pcCommandString) {
  ...
  const char *ethMode = (ethConfig.dhcpMode == DHCP_STATIC)    ? "STATIC"
  ...
  if (pcParameter == NULL || lParameterLength == 0 ||
    ...
    snprintf(pcWriteBuffer, xWriteBufferLen,
             ...
             (unsigned long)ethConfig.portNumber, ethConfig.subnetMask[0],
             ethConfig.subnetMask[1], ethConfig.subnetMask[2],
             ethConfig.subnetMask[3], ethConfig.gatewayAddress[0],
  ...
  param[lParameterLength < (BaseType_t)(sizeof(param) - 1)
            ? lParameterLength
            : (sizeof(param) - 1)] = '\0';
  ...
  if (strncmp(param, "ip", 2) == 0) {
    ...
    snprintf(pcWriteBuffer, xWriteBufferLen, "Mask: %u.%u.%u.%u\r\n",
             ethConfig.subnetMask[0], ethConfig.subnetMask[1],
             ethConfig.subnetMask[2], ethConfig.subnetMask[3]);
  } else if (strncmp(param, "gateway", 7) == 0) {
    snprintf(pcWriteBuffer, xWriteBufferLen, "Gateway: %u.%u.%u.%u\r\n",
             ethConfig.gatewayAddress[0], ethConfig.gatewayAddress[1],
             ethConfig.gatewayAddress[2], ethConfig.gatewayAddress[3]);
...
BaseType_t prvResetCommand(char *pcWriteBuffer, size_t xWriteBufferLen,
                           const char *pcCommandString) {
  const char *pcParameter;
  BaseType_t lParameterLength;
