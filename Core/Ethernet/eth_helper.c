...
int32_t xTCPSendAndReceive(const char *pcTxBuffer, size_t txLen,
                           char *pcRxBuffer, size_t rxBufLen,
  ...
  /* Use the same address family constant as used when creating the socket. */
  xRemoteAddress.sin_family = FREERTOS_AF_INET;

  printf("start creating tcp socket \n");
  ...
  printf("Tcp socket being created successfuly \n");
  ...
  for (;;) {
    ...
    BaseType_t xBytesReceived =
        FreeRTOS_recv(xSocket, &pcRxBuffer[alreadyReceived],
                      (size_t)(rxBufLen - alreadyReceived), 0);
    if (xBytesReceived > 0) {
...
int32_t xTCPSend(const char *pcTxBuffer, size_t txLen, uint16_t port) {
  Socket_t xSocket = FREERTOS_INVALID_SOCKET;
  struct freertos_sockaddr xRemoteAddress;
  size_t alreadyTransmitted = 0;
  int32_t result = -1;
  ...
  memset(&xRemoteAddress, 0, sizeof(xRemoteAddress));
  xRemoteAddress.sin_port = FreeRTOS_htons(port);
  xRemoteAddress.sin_address.ulIP_IPv4 =
      FreeRTOS_inet_addr_quick(192, 168, 0, 111);
