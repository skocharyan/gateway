...
// static void Convert_To_Str(uint32_t* Data, char* Buf);
// static void Flash_Write_NUM(uint32_t StartSectorAddress, float Num);
// static float Flash_Read_NUM(uint32_t StartSectorAddress);
...
uint32_t Flash_Write_Data(uint32_t StartSectorAddress, uint32_t* Data,
    ...
    //    FLASH->SR = FLASH_SR_EOP; /* Clear SR register */
    __HAL_FLASH_CLEAR_FLAG(FLASH->SR);
    ...
    /* Note: If an erase operation in Flash memory also concerns data in the
       data or instruction cache, you have to make sure that these data are
       rewritten before they are accessed during code execution. If this cannot
       be done safely, it is recommended to flush the caches by setting the
    ...
    while (sofar < numberofwords) {
        if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, StartSectorAddress,
            Data[sofar]) == HAL_OK) {
            StartSectorAddress += 4; // use StartPageAddress += 2 for half word
            // and 8 for double word
            sofar++;
        }
        else {
...
void Flash_Read_Data(uint32_t StartSectorAddress, uint32_t* RxBuf, uint16_t numberofwords) {
    while (numberofwords > 0) { // Ensure the loop runs only while numberofwords is positive
        *RxBuf = *(__IO uint32_t*)StartSectorAddress;
        StartSectorAddress += 4;
        RxBuf++;
        numberofwords--; // Decrement safely
    }
}
...
//     int numberofbytes =
//         ((strlen((char*)Data) / 4) + ((strlen((char*)Data) % 4) != 0)) * 4;
